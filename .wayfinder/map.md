...
| **Kotlin port, modules 1-2 + SignalSource real and verified** | Signal chain + PPG/fusion, line-for-line ports of the Dart source. Found `ppg_reference.py` silently drifted from `ppg.dart` on two formulas (percentile method, "median" definition) — `validate_ppg.py`'s range-based checks never could have caught it; flagged, not fixed. Module 3 (Policy) paused by user request; `SignalSource`/`ReplaySource` pulled forward instead — a real PhysioNet AF recording (A00004) bundled and replayed through the real pipeline: SQI 0.934, irregularityScore **0.8136** (gate 0.5). `gradle testDebugUnitTest`: 9/9 passing. See tickets 019, 009 |
