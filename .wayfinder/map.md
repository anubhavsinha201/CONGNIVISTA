...
| **Spoken Tamil — 6 real audio clips generated, all text unreviewed** | `android/scripts/generate_tamil_audio.py` against a real ElevenLabs account produced 6 genuine MP3s (verified `ID3` headers) for the 5 tiers + supporting line. `tamil_strings_DRAFT.json` now drafts all 33 `EXPLANATION_KEYS` too, but every string — Tamil and English — is AI-written, not reviewed by a native speaker or clinician; ticket 011 is unchanged in substance, just has something concrete to react to now. Dynamic explanation strings (`{hr}`, `{sqi}`...) deliberately excluded from cloud TTS — that's Android on-device `TextToSpeech`'s job, not ElevenLabs'. See tickets 011, 015 |
