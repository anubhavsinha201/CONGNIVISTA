...
| **Kotlin port: modules 1, 2, 5, 7, 8 done; 3 deliberately narrowed to types; 6 open** | Signal chain + PPG/fusion + record/patient-history + SignalSource + sync all fully built. Sync's real `HttpSyncClient` (HttpURLConnection, chosen for compatibility with pre-API-34 budget phones) + `SyncEngine` orchestration verified against the live Atlas-backed server — a real flush, independently confirmed via `GET /v1/queue`, test record cleaned up after. Audio playback wiring (`SequencePlayer`/`ClipPlayer`) built and tested with a fake player; the real `MediaPlayer` adapter is honestly unverified until a device/emulator exists, same as ticket 013's firmware before flashing. Explanation (4) still only has Policy's *types*, not `decide()` — the boundary is named in `PolicyTypes.kt`'s header. A real PhysioNet AF recording (A00004) replayed through the real pipeline: SQI 0.934, irregularityScore **0.8136** (gate 0.5). `gradle testDebugUnitTest`: **71 tests, 70 passing + 1 correctly `@Ignore`d live-server test**, 13 classes. See ticket 019 |
