...
| **Kotlin port, modules 1-2 real and verified** | Signal chain + PPG/fusion, line-for-line ports of the Dart source. `gradle testDebugUnitTest`: 7/7 passing at 1e-6. Along the way, found `ppg_reference.py` silently drifted from `ppg.dart` on two formulas (percentile method, "median" definition) — `validate_ppg.py`'s range-based checks never could have caught it. Not fixed in the Python reference; flagged in ticket 019 for a human decision. See ticket 019 |
